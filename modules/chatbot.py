...
class QwenChatEngine:
   ...
   def _build_prompt(self) -> str:
       ...
       prompt = self.tokenizer.apply_chat_template(
           ...
           enable_thinking=False, # 始终禁用思考模式
       )
       return prompt
   ...
   def _message_token_len(self, msg: dict) -> int:
       ...
       # print(">>> content repr:", repr(text[:200])) # 调试信息，可以注释掉
       # 3️⃣ 检查是否含有代理字符
   ...
   def chat(
       ...
       **gen_kwargs,
       ...
       prompt = self._build_prompt() # 移除 enable_thinking 參數
       ...
       # 移除：不再進行思考段切分，整段直接作為答案
       answer_text: str = full_text.strip()

       # 10️⃣ 把最終答案寫回歷史
       self.conversation.append({"role": "assistant", "content": answer_text})

       return {"answer": answer_text} # 移除 "thinking" 鍵
